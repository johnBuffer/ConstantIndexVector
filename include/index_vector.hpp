...
namespace civ
{
    ...
    inline void Vector<T>::erase(ID id)
    {
        ...
        if (data_index >= data.size()) { return; }
        // Swap the object at the end
        const uint64_t last_id = metadata.back().rid;
        std::swap(data.back(), data[data_index]);
        std::swap(metadata.back(), metadata[data_index]);
    ...
    {
        const uint64_t id = data.size();
        ids.push_back(id);
        metadata.push_back({id, op_count++});
        return { id, id };
    }
    ...
    {
        for (uint64_t data_index{ 0 }; data_index < data.size();) {
    ...
    void Vector<T>::foreach(TCallback &&callback) {
        ...
        const uint64_t current_size = data.size();
